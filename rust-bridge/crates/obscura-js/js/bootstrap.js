...
globalThis.removeEventListener = function (type, fn) {
	if (globalThis.__windowListeners[type]) {
		globalThis.__windowListeners[type] = globalThis.__windowListeners[
			type
		].filter((h) => h !== fn);
	}
};
...
const _origStackDesc = Object.getOwnPropertyDescriptor(
	Error.prototype,
	"stack",
);
...
class Element extends Node {
	...
	removeEventListener(type, handler) {
		...
		if (_eventRegistry[key] && _eventRegistry[key][type]) {
			_eventRegistry[key][type] = _eventRegistry[key][type].filter(
				(h) => h !== handler,
			);
		}
	}
	...
	blur() {
		if (globalThis.__obscura_focused === this)
	...
	get type() {
		return (
			this.getAttribute("type") || (this.localName === "input" ? "text" : "")
		);
	}
	set type(v) {
	...
	submit(submitter) {
		...
		if (method === "POST") {
			...
			Deno.core.ops.op_navigate(
				targetUrl + (encoded ? sep + encoded : ""),
				"GET",
				"",
			);
		}
...
function _installWasmStreamingFallback() {
	...
	if (
		...
		WebAssembly.instantiateStreaming.__obscuraFallback
	)
		return;
...
globalThis.fetch = async (input, init = {}) => {
	...
	if (parsed.corsBlocked) {
		throw new TypeError(
			"Failed to fetch: " + (parsed.corsError || "CORS error"),
		);
	}
	const respType =
		parsed.status === 0
	...
	return new Proxy(style, {
		get(target, prop) {
			...
			if (prop === "getPropertyValue") {
				return (name) => {
					const v = target.getPropertyValue
					...
					return (
						defaults[name] ||
						defaults[name.replace(/-([a-z])/g, (_, c) => c.toUpperCase())] ||
						""
					);
				};
			}
...
globalThis.__notifyMutation = function (
	type,
	target_nid,
	addedNodes,
	removedNodes,
...
class _Canvas2D {
	...
	_parseColor(css) {
		...
		if (m)
			return [
				...
				+m[2],
				+m[3],
				m[4] !== undefined ? Math.round(+m[4] * 255) : 255,
			];
		const named = {
			red: [255, 0, 0, 255],
			green: [0, 128, 0, 255],
	...
	_setPixel(x, y, r, g, b, a) {
		...
		this._buf[idx + 0] = Math.round(
			r * alpha + this._buf[idx + 0] * (1 - alpha),
		);
		this._buf[idx + 1] = Math.round(
			g * alpha + this._buf[idx + 1] * (1 - alpha),
		);
		this._buf[idx + 2] = Math.round(
			b * alpha + this._buf[idx + 2] * (1 - alpha),
		);
		this._buf[idx + 3] = Math.min(
			255,
			Math.round(a * alpha + this._buf[idx + 3] * (1 - alpha)),
		);
	}
	fillRect(x, y, w, h) {
		...
		for (let i = 0; i < str.length; i++) {
			...
			for (let row = 0; row < 7; row++) {
				for (let col = 0; col < 5; col++) {
					const on =
						(_fpRand(code * 100 + row * 10 + col) > 0.45 &&
							row > 0 &&
							row < 6 &&
							col > 0 &&
							col < 4) ||
					...
					if (on) {
						for (let sy = 0; sy < scale; sy++) {
							for (let sx = 0; sx < scale; sx++) {
								this._setPixel(
									cx + col * scale + sx,
									Math.round(y) - 7 * scale + row * scale + sy,
									r,
									g,
									b,
									a,
...
globalThis.OfflineAudioContext = class OfflineAudioContext extends (
	AudioContext
) {
...
if (typeof OffscreenCanvas === "undefined") {
	globalThis.OffscreenCanvas = class OffscreenCanvas {
		...
		getContext(type) {
			return (
				globalThis.document?.createElement("canvas")?.getContext(type) || null
			);
		}
		convertToBlob() {
...
if (
	...
	ShadowRoot.prototype.elementFromPoint = function (x, y) {
		return Document.prototype.elementFromPoint.call(
...
globalThis.__obscura_init = function () {
	...
	const toHide = Object.keys(globalThis).filter(
		(k) => k.startsWith("_") || k.includes("obscura") || k.includes("Obscura"),
	);
