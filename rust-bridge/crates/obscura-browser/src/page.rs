...
impl Page {
    ...
    pub fn evaluate(&mut self, expression: &str) -> serde_json::Value {
        if let Some(js) = &mut self.js {
            js.evaluate(expression).unwrap_or(serde_json::Value::Null)
        } else {
    ...
    pub async fn evaluate_for_cdp(
        ...
        if let Some(js) = &mut self.js {
            match js.evaluate_for_cdp(expression, return_by_value, await_promise).await {
                Ok(info) => info,
                Err(e) => {
                    tracing::debug!("evaluate error: {}", e);
                    obscura_js::runtime::RemoteObjectInfo {
    ...
    pub async fn execute_preload_script(&mut self, source: &str) -> Result<(), String> {
        if let Some(js) = &mut self.js {
            js.execute_script("<preload>", source).await.map(|_| ())
        } else {
