...
- **`ScrapeResolver` instancie `new BxcDB()` au chargement du module** →
...
bun run db:push      # drizzle-kit push -> data/bxc.sqlite
...
(pas de dépendance npm sur bxc). `tsconfig.json` `include` ajoute `../../src`.
