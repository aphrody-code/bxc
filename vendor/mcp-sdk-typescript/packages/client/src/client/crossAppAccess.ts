...
export interface DiscoverAndRequestJwtAuthGrantOptions
	extends Omit<RequestJwtAuthGrantOptions, "tokenEndpoint"> {
	/**
...
export async function requestJwtAuthorizationGrant(
	...
	if (!response.ok) {
		...
		if (parseResult.success) {
			...
			throw new Error(
				`Token exchange failed: ${error}${error_description ? ` - ${error_description}` : ""}`,
			);
		}

		throw new Error(
			`Token exchange failed with status ${response.status}: ${JSON.stringify(errorBody)}`,
		);
	}

	const parseResult = IdJagTokenExchangeResponseSchema.safeParse(
		await response.json(),
	);
	if (!parseResult.success) {
		throw new Error(
 ...
 * ```
 */
export async function discoverAndRequestJwtAuthGrant(
	...
	const metadata = await discoverAuthorizationServerMetadata(String(idpUrl), {
		fetchFn,
	});
...
export async function exchangeJwtAuthGrant(options: {
	...
	if (!response.ok) {
		...
		if (parseResult.success) {
			const { error, error_description } = parseResult.data;
			throw new Error(
				`JWT grant exchange failed: ${error}${error_description ? ` - ${error_description}` : ""}`,
			);
		}

		throw new Error(
			`JWT grant exchange failed with status ${response.status}: ${JSON.stringify(errorBody)}`,
		);
	}
