...
export class StreamableHTTPClientTransport implements Transport {
	...
	private async _startOrAuthSse(
		...
		try {
			...
			const types = [
				...(userAccept?.split(",").map((s) => s.trim().toLowerCase()) ?? []),
				"text/event-stream",
			];
	...
	private _scheduleReconnection(
				...
				try {
					...
					this.onerror?.(
						scheduleError instanceof Error
							? scheduleError
							: new Error(String(scheduleError)),
					);
				}
			});
		};
		...
		if (this._reconnectionScheduler) {
			const cancel = this._reconnectionScheduler(
				reconnect,
				delay,
				attemptCount,
			);
			this._cancelReconnection =
				typeof cancel === "function" ? cancel : undefined;
		} else {
