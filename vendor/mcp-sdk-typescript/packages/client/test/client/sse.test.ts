...
import type { Mock, Mocked, MockedFunction, MockInstance } from "bun:test";
...
describe("SSEClientTransport", () => {
	...
	describe("header handling", () => {
		...
		it("passes custom headers to fetch requests", async () => {
			...
			try {
				...
				const calledHeaders = (globalThis.fetch as Mock).mock.calls[0]![1]
					.headers;
				...
				const updatedHeaders = (globalThis.fetch as Mock).mock.calls[1]![1]
					.headers;
	...
	describe("auth handling", () => {
		const authServerMetadataUrls = new Set([
		...
		it("invalidates all credentials on OAuthErrorCode.InvalidClient during token refresh", async () => {
			...
			const expectedError = new OAuthError(
			...
			const server = createServer((req, res) => {
				...
				if (req.url === "/token" && req.method === "POST") {
					res
						...
						.end(JSON.stringify(expectedError.toResponseObject()));
					return;
				}
	...
	describe("minimal AuthProvider (non-OAuth)", () => {
		...
		it("enforces circuit breaker on double-401: onUnauthorized called once, then throws SdkError", async () => {
			...
			expect((error as SdkError).code).toBe(
		...
		it("throws when finishAuth is called with a non-OAuth AuthProvider", async () => {
			...
			await expect(transport.finishAuth("auth-code")).rejects.toThrow(
