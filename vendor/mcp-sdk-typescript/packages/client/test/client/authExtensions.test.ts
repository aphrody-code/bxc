...
describe("createPrivateKeyJwtAuth", () => {
	...
	it("throws when jose cannot import an invalid RSA PEM key", async () => {
		...
		).rejects.toThrow(/cannot be part of a valid base64|Invalid character/);
	});
...
describe("CrossAppAccessProvider", () => {
	...
	it("passes custom fetchFn to assertion callback", async () => {
		...
		const wrappedFetch = vi.fn((...args: Parameters<typeof fetchMock>) =>
			fetchMock(...args),
		);
	...
	it("throws error for redirectToAuthorization (not used in jwt-bearer)", () => {
		...
		expect(() => provider.redirectToAuthorization()).toThrow(
			"redirectToAuthorization is not used for jwt-bearer flow",
		);
	});
	...
	it("throws error for codeVerifier (not used in jwt-bearer)", () => {
		...
		expect(() => provider.codeVerifier()).toThrow(
			"codeVerifier is not used for jwt-bearer flow",
		);
	});
