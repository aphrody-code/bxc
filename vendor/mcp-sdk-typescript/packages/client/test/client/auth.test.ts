...
describe("OAuth Authorization", () => {
	...
	describe("discoverOAuthProtectedResourceMetadata", () => {
		...
		])("falls back to root discovery when path-aware discovery returns %d", async (statusCode) => {
	...
	describe("startAuthorization", () => {
		...
		it.each([
			validMetadata,
			validOpenIdMetadata,
		])("uses metadata authorization_endpoint when provided", async (baseMetadata) => {
			const { authorizationUrl } = await startAuthorization(
				"https://auth.example.com",
				{
		...
		it.each([
			validMetadata,
			validOpenIdMetadata,
		])("validates response type support", async (baseMetadata) => {
			const metadata = {
				...baseMetadata,
		...
		it.each([
			validMetadata,
			validOpenIdMetadata,
		])("assumes supported code challenge methods includes S256 if absent", async (baseMetadata) => {
			const metadata = {
				...baseMetadata,
				response_types_supported: ["code"],
		...
		it.each([
			validMetadata,
			validOpenIdMetadata,
		])("validates supported code challenge methods includes S256 if present", async (baseMetadata) => {
			const metadata = {
				...baseMetadata,
				response_types_supported: ["code"],
	...
	describe("validateClientMetadataUrl", () => {
		...
		it("passes for valid HTTPS URL with multi-segment path", () => {
			expect(() =>
				validateClientMetadataUrl("https://example.com/clients/metadata.json"),
			).not.toThrow();
		});
		...
		it("throws OAuthError for HTTP URL", () => {
			expect(() =>
				validateClientMetadataUrl(
					"http://client.example.com/.well-known/oauth-client",
				),
			).toThrow(OAuthError);
			try {
				validateClientMetadataUrl(
