...
describe("registerTool/registerPrompt accept raw Zod shape (auto-wrapped)", () => {
	...
	it("registerTool accepts a raw shape for outputSchema and auto-wraps it", () => {
		...
		server.registerTool(
			"out",
			{ inputSchema: { n: z.number() }, outputSchema: { result: z.string() } },
			async ({ n }) => ({
	...
	it("registerPrompt accepts a raw shape for argsSchema", () => {
		...
		server.registerPrompt(
			...
			async ({ topic }) => ({
				messages: [
					{
						role: "user" as const,
						content: { type: "text" as const, text: topic },
					},
				],
			}),
		);
		...
		const prompts = (
			server as unknown as {
				_registeredPrompts: Record<string, { argsSchema?: unknown }>;
			}
		)._registeredPrompts;
	...
	it("registerPrompt raw shape accepts completable() fields (v1 pattern)", () => {
		...
		server.registerPrompt(
			...
			{
				argsSchema: {
					language: completable(z.string(), (v) =>
						["typescript", "python"].filter((l) => l.startsWith(v)),
					),
				},
			},
			async ({ language }) => ({
	...
	it("callback receives validated, typed args end-to-end via tools/call", async () => {
		...
		await vi.waitFor(() =>
			expect(responses.some((r) => "id" in r && r.id === 2)).toBe(true),
		);
