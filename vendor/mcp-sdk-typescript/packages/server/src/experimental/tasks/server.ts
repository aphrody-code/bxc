...
export class ExperimentalServerTasks {
	...
	requestStream<M extends RequestMethod>(
		...
		return this._module.requestStream(
			request as Request,
			resultSchema,
			options,
		) as AsyncGenerator<ResponseMessage<ResultTypeMap[M]>, void, void>;
	}
	...
	async getTaskResult(
		...
		return this._module.getTaskResult(
			{ taskId },
			GetTaskPayloadResultSchema,
			options,
		);
