 ...
 */
export type ServerTasksCapabilityWithRuntime = NonNullable<
...
export class Server extends Protocol<ServerContext> {
	...
	constructor(
		...
		this._jsonSchemaValidator =
			options?.jsonSchemaValidator ?? new DefaultJsonSchemaValidator();
	...
	private _registerLoggingHandler(): void {
		this.setRequestHandler("logging/setLevel", async (request, ctx) => {
			const transportSessionId: string | undefined =
				ctx.sessionId ||
				(ctx.http?.req?.headers.get("mcp-session-id") as string) ||
				undefined;
		...
		return {
			...
			mcpReq: {
				...
				log: (level, data, logger) =>
					this.sendLoggingMessage({ level, data, logger }),
	...
	protected override _wrapHandler(
		...
		return async (request, ctx) => {
			...
			if (params.task) {
				const taskValidationResult = parseSchema(
					CreateTaskResultSchema,
					result,
				);
	...
	private async _oninitialize(
		...
		const protocolVersion = this._supportedProtocolVersions.includes(
			requestedVersion,
		)
			? requestedVersion
	...
	async createMessage(
		...
		if (params.messages.length > 0) {
			...
			const previousMessage =
				params.messages.length > 1 ? params.messages.at(-2) : undefined;
			...
			if (hasPreviousToolUse) {
				const toolUseIds = new Set(
					previousContent
						...
						.map((c) => (c as ToolUseContent).id),
				);
				const toolResultIds = new Set(
					lastContent
						.filter((c) => c.type === "tool_result")
						.map((c) => (c as ToolResultContent).toolUseId),
				);
				if (
	...
	async sendLoggingMessage(
		...
		if (
			...
			!this.isMessageIgnored(params.level, sessionId)
		) {
