...
export class WebStandardStreamableHTTPServerTransport implements Transport {
	...
	constructor(options: WebStandardStreamableHTTPServerTransportOptions = {}) {
		...
		this._supportedProtocolVersions =
			options.supportedProtocolVersions ?? SUPPORTED_PROTOCOL_VERSIONS;
	}
	...
	private async handlePostRequest(
		...
		try {
			...
			const isInitializationRequest = messages.some((element) =>
				isInitializeRequest(element),
			);
			...
			if (allResponsesReady) {
				...
				if (this._enableJsonResponse && stream.resolveJson) {
					...
					if (responses.length === 1) {
						stream.resolveJson(
							Response.json(responses[0], { status: 200, headers }),
						);
					} else {
						stream.resolveJson(
							Response.json(responses, { status: 200, headers }),
						);
					}
				} else {
					// End the SSE stream
