...
export class McpServer {
		...
		this.server.setRequestHandler(
			...
			async (request, ctx): Promise<CallToolResult | CreateTaskResult> => {
				...
				try {
					...
					const isTaskHandler =
						"createTask" in
						(tool.handler as AnyToolHandler<StandardSchemaWithJSON>);
	...
	>(tool: ToolType, args: Args, toolName: string): Promise<Args> {
		...
		const parseResult = await validateStandardSchema(
			tool.inputSchema,
			args ?? {},
		);
	...
	private async handleAutomaticTaskPolling<RequestT extends CallToolRequest>(
		...
		const createTaskResult = (await tool.executor(
			args,
			ctx,
		)) as CreateTaskResult;
	...
	private setResourceRequestHandlers() {
		...
		this.server.registerCapabilities({
			resources: {
				listChanged:
					this.server.getCapabilities().resources?.listChanged ?? true,
			},
		});
	...
	private setPromptRequestHandlers() {
		...
		this.server.setRequestHandler(
			"prompts/get",
			async (request, ctx): Promise<GetPromptResult> => {
	...
	private _createRegisteredPrompt(
		...
		const registeredPrompt: RegisteredPrompt = {
			...
			update: (updates) => {
				...
				if (updates.callback !== undefined) {
					currentCallback = updates.callback as PromptCallback<
						StandardSchemaWithJSON | undefined
					>;
					needsHandlerRegen = true;
				}
				if (needsHandlerRegen) {
					registeredPrompt.handler = createPromptHandler(
						name,
						currentArgsSchema,
						currentCallback,
					);
		...
		const registeredTool: RegisteredTool = {
			...
			update: (updates) => {
				...
				if (updates.callback !== undefined) {
					...
					currentHandler = updates.callback as AnyToolHandler<
						StandardSchemaWithJSON | undefined
					>;
					needsExecutorRegen = true;
				}
				if (needsExecutorRegen) {
					registeredTool.executor = createToolExecutor(
	...
	registerTool(
		...
		cb:
			...
			| LegacyToolCallback<ZodRawShape>,
	): RegisteredTool {
	...
	registerPrompt(
		...
		cb:
			| PromptCallback<StandardSchemaWithJSON>
			| LegacyPromptCallback<ZodRawShape>,
	): RegisteredPrompt {
...
export type LegacyPromptCallback<Args extends ZodRawShape | undefined> =
	Args extends ZodRawShape
		? (
				args: InferRawShape<Args>,
				ctx: ServerContext,
	...
	if (isTaskHandler) {
		...
		return async (args, ctx) => {
			...
			const taskCtx: CreateTaskServerContext = {
				...ctx,
				task: { store: ctx.task.store, requestedTtl: ctx.task?.requestedTtl },
			};
			if (inputSchema) {
