...
} from "../src/middleware/hostHeaderValidation.js";
...
describe("@modelcontextprotocol/express", () => {
	describe("hostHeaderValidation", () => {
		...
		test("should handle multiple allowed hostnames", () => {
			const middleware = hostHeaderValidation([
				"localhost",
				"127.0.0.1",
				"myapp.local",
			]);
			const {
				...
				res: res1,
				next: next1,
			} = createMockReqResNext("127.0.0.1:8080");
			const {
				req: req2,
				res: res2,
				next: next2,
			} = createMockReqResNext("myapp.local");
	...
	describe("createMcpExpressApp", () => {
		...
		test("should warn when binding to :: without allowedHosts", () => {
			...
			expect(warn).toHaveBeenCalledWith(
				expect.stringContaining(
					"Warning: Server is binding to :: without DNS rebinding protection",
				),
			);
