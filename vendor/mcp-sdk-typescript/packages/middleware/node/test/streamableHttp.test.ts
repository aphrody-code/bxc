...
describe("Zod v4", () => {
	...
	async function createTestServer(config?: TestServerConfig): Promise<{
		...
		const server = createServer(async (req, res) => {
			try {
				await (config.customRequestHandler
					? config.customRequestHandler(req, res)
			...
			async ({ active }, ctx): Promise<CallToolResult> => {
				return {
					content: [
						{
							type: "text",
							text: `${active ? "Active" : "Inactive"} profile from token: ${ctx.http?.authInfo?.token}!`,
						},
					],
				};
			},
				...
				async ({ name }, ctx): Promise<CallToolResult> => {
					...
					return {
						content: [
							...
							{
								type: "text",
								text: `${JSON.stringify(serializedRequestInfo)}`,
							},
						],
					};
				},
	...
	describe("NodeStreamableHTTPServerTransport with JSON Response Mode", () => {
		...
		it("should return JSON response for batch requests", async () => {
			...
			const listResponse = results.find(
				(r: { id?: RequestId }) => r.id === "batch-1",
			);
			const callResponse = results.find(
				(r: { id?: RequestId }) => r.id === "batch-2",
			);
