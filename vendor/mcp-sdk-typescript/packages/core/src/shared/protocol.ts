...
export abstract class Protocol<ContextT extends BaseContext> {
	...
	constructor(private _options?: ProtocolOptions) {
		...
		this._taskManager = _options?.tasks
			? new TaskManager(_options.tasks)
			: new NullTaskManager();
	...
	private _bindTaskManager(): void {
		...
		const host: TaskManagerHost = {
			...
			notification: (notification, options) =>
				this.notification(notification, options),
			...
			assertTaskHandlerCapability: (method) =>
				this.assertTaskHandlerCapability(method),
		};
	...
	private _onrequest(request: JSONRPCRequest, extra?: MessageExtraInfo): void {
		...
		const inboundCtx = {
			...
			sendRequest: <U extends StandardSchemaV1>(
				...
				this._requestWithSchema(r, resultSchema, {
					...options,
					relatedRequestId: request.id,
				}),
		};
		...
		const taskResult = this._taskManager.processInboundRequest(
			request,
			inboundCtx,
		);
		...
		const baseCtx: BaseContext = {
			...
			mcpReq: {
				...
				send: ((
					r: Request,
					schemaOrOptions?: StandardSchemaV1 | TaskRequestOptions,
	...
	private _onresponse(response: JSONRPCResponse | JSONRPCErrorResponse): void {
		...
		const taskResult = this._taskManager.processInboundResponse(
			response,
			messageId,
		);
		...
		if (isJSONRPCResultResponse(response)) {
			...
			const error = ProtocolError.fromError(
				response.error.code,
	...
	async notification(
		...
		const taskResult = await this._taskManager.processOutboundNotification(
			notification,
			options,
		);
		const queued = taskResult.queued;
		const jsonrpcNotification = taskResult.queued
			? undefined
			: taskResult.jsonrpcNotification;
	...
	setRequestHandler<M extends RequestMethod>(
		...
		handler: (
			...
			ctx: ContextT,
		) => ResultTypeMap[M] | Promise<ResultTypeMap[M]>,
	): void;
	setRequestHandler<
	...
	setRequestHandler(
		...
		schemasOrHandler:
			...
			| ((request: unknown, ctx: ContextT) => Result | Promise<Result>),
		...
		if (typeof schemasOrHandler === "function") {
			...
			stored = (request, ctx) =>
				Promise.resolve(schemasOrHandler(schema.parse(request), ctx));
	...
	setNotificationHandler(
		...
		if (typeof schemasOrHandler === "function") {
			...
			this._notificationHandlers.set(method, (notification) =>
				Promise.resolve(schemasOrHandler(schema.parse(notification))),
			);
			return;
		}
...
type InferHandlerResult<R extends StandardSchemaV1 | undefined> =
