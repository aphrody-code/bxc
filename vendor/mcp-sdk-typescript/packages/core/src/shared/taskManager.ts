...
export class TaskManager {
	...
	bind(host: TaskManagerHost): void {
		...
		if (this._taskStore) {
			...
			host.registerHandler("tasks/result", async (request, ctx) => {
				const params = request.params as { taskId: string };
				return await this.handleGetTaskPayload(
					params.taskId,
					ctx.sessionId,
					ctx.mcpReq.signal,
					async (message) => {
	...
	protected get _requireHost(): TaskManagerHost {
		if (!this._host) {
			throw new ProtocolError(
				...
				"TaskManager is not bound to a Protocol host — call bind() first",
			);
		}
		return this._host;
	}
	...
	async *requestStream<T extends AnyObjectSchema>(
		...
		try {
			const createResult = await host.request(
				request,
				CreateTaskResultSchema,
				options,
			);
	...
	private async handleGetTaskPayload(
		...
		const handleTaskResult = async (): Promise<Result> => {
			...
			const result = await this._requireTaskStore.getTaskResult(
				taskId,
				sessionId,
			);
	...
	private async handleListTasks(
		...
		try {
			const { tasks, nextCursor } = await this._requireTaskStore.listTasks(
				cursor,
				sessionId,
			);
	...
	private async handleCancelTask(
		...
		try {
			...
			await this._requireTaskStore.updateTaskStatus(
				taskId,
				"cancelled",
			...
			const cancelledTask = await this._requireTaskStore.getTask(
				taskId,
				sessionId,
			);
			if (!cancelledTask) {
	...
	private extractInboundTaskContext(
		...
		const relatedTaskId = (
			request.params?._meta as Record<string, { taskId?: string }> | undefined
		)?.[RELATED_TASK_META_KEY]?.taskId;
	...
	private wrapSendRequest(
		...
		return async <V extends StandardSchemaV1>(
			request: Request,
			resultSchema: V,
			options?: TaskRequestOptions,
		) => {
	...
	private async routeResponse(
		...
		await (isJSONRPCErrorResponse(message)
			...
			: this._enqueueTaskMessage(
					...
					{
						type: "response",
						message: message as JSONRPCResultResponse,
						timestamp: Date.now(),
					},
					sessionId,
				));
	...
	processOutboundRequest(
		...
		const queued = this.prepareOutboundRequest(
			jsonrpcRequest,
			options,
			messageId,
			responseHandler,
	...
	processInboundResponse(
		...
		const preserveProgress = this.shouldPreserveProgressHandler(
			response,
			messageId,
		);
	...
	private async _enqueueTaskMessage(
		...
		await this._taskMessageQueue.enqueue(
			...
			message,
			sessionId,
			this._options.maxTaskQueueSize,
		);
	}
	...
	private async _waitForTaskUpdate(
		...
		const interval =
			pollInterval ?? this._options.defaultTaskPollInterval ?? 1000;
...
export class NullTaskManager extends TaskManager {
	...
	override processInboundRequest(
		request: JSONRPCRequest,
		ctx: InboundContext,
	): InboundResult {
		const hasTaskCreationParams =
			isTaskAugmentedRequestParams(request.params) && !!request.params.task;
		return {
