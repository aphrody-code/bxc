...
import type {
	SpecTypeName,
	SpecTypes,
} from "../../src/types/specTypeSchema.js";
...
describe("specTypeSchemas", () => {
	it("returns a StandardSchemaV1Sync validator that accepts valid values", () => {
		const result = specTypeSchemas.Implementation["~standard"].validate({
			name: "x",
			version: "1.0.0",
		});
	...
	it("covers JSON-RPC envelope types", () => {
		const ok = specTypeSchemas.JSONRPCRequest["~standard"].validate({
			jsonrpc: "2.0",
			id: 1,
			method: "ping",
		});
	...
	it("covers OAuth types from shared/auth.ts", () => {
		const ok = specTypeSchemas.OAuthTokens["~standard"].validate({
			access_token: "x",
			token_type: "Bearer",
		});
		expect(ok.issues).toBeUndefined();
		const bad = specTypeSchemas.OAuthTokens["~standard"].validate({
			token_type: "Bearer",
		});
...
describe("isSpecType", () => {
	it("CallToolResult — accepts valid, rejects invalid/null/primitive", () => {
		expect(
			isSpecType.CallToolResult({ content: [{ type: "text", text: "hi" }] }),
		).toBe(true);
	...
	it("Tool — accepts valid, rejects missing inputSchema", () => {
		expect(
			isSpecType.Tool({ name: "echo", inputSchema: { type: "object" } }),
		).toBe(true);
	...
	it("ResourceTemplate — accepts valid, rejects missing uriTemplate", () => {
		expect(
			isSpecType.ResourceTemplate({ name: "r", uriTemplate: "file:///{path}" }),
		).toBe(true);
...
describe("SpecTypeName / SpecTypes (type-level)", () => {
	...
	it("SpecTypes[K] matches the named export type", () => {
		...
		expectTypeOf<
			SpecTypes["ResourceTemplate"]
		>().toEqualTypeOf<ResourceTemplateType>();
	});
});
...
describe("SPEC_SCHEMA_KEYS allowlist", () => {
	...
	it("covers every public protocol schema in schemas.ts (drift guard)", () => {
		...
		const allProtocolSchemas = Object.keys(schemas).filter(
			(k) => k.endsWith("Schema") && /^[A-Z]/.test(k),
		);
