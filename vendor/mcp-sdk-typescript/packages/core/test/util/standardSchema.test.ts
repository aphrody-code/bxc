...
describe("standardSchemaToJsonSchema", () => {
	...
	test("throws for schemas with explicit non-object type", () => {
		expect(() => standardSchemaToJsonSchema(z.string(), "input")).toThrow(
			/must describe objects/,
		);
		expect(() =>
			standardSchemaToJsonSchema(z.array(z.string()), "input"),
		).toThrow(/must describe objects/);
		expect(() => standardSchemaToJsonSchema(z.number(), "input")).toThrow(
			/must describe objects/,
		);
	});
