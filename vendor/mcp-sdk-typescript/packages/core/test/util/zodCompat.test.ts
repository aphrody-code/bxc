...
describe("normalizeRawShapeSchema", () => {
	...
	test("throws TypeError for an invalid object that is neither raw shape nor Standard Schema", () => {
		expect(() =>
			normalizeRawShapeSchema({ a: "not a zod schema" } as never),
		).toThrow(TypeError);
	});
	...
	test("passes through a zod 4.0-4.1 schema so standardSchemaToJsonSchema can apply its z.toJSONSchema fallback", () => {
		...
		const { jsonSchema: _drop, ...stdNoJson } = real[
			"~standard"
		] as unknown as Record<string, unknown>;
		void _drop;
		Object.defineProperty(real, "~standard", {
			value: { ...stdNoJson, vendor: "zod" },
			configurable: true,
		});
	...
	test("throws actionable TypeError for a raw shape with Zod v3 fields", () => {
		expect(() =>
			normalizeRawShapeSchema({ a: mockZodV3String() } as never),
		).toThrow(/Zod v4 schemas.*Got a Zod v3 field schema/);
	});
	test("throws the intended TypeError (not Object.values crash) for null input", () => {
		expect(() => normalizeRawShapeSchema(null as never)).toThrow(
			/must be a Standard Schema/,
		);
	});
});
