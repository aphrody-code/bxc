...
import {
	...
	InMemoryTaskStore,
} from "../../src/experimental/tasks/stores/inMemory.js";
...
describe("InMemoryTaskStore", () => {
	...
	describe("updateTaskStatus", () => {
		...
		it("should throw if task not found", async () => {
			await expect(
				store.updateTaskStatus("non-existent", "working"),
		...
		describe("status lifecycle validation", () => {
			...
			it("should reject transition from completed to any other status", async () => {
				...
				await expect(
					store.updateTaskStatus(taskId, "input_required"),
	...
	describe("storeTaskResult", () => {
		...
		it("should throw if task not found", async () => {
			await expect(
				store.storeTaskResult("non-existent", "completed", {}),
			).rejects.toThrow("Task with ID non-existent not found");
		});
		...
		it("should reject storing result for task already in completed status", async () => {
			...
			await expect(
				store.storeTaskResult(taskId, "completed", secondResult),
		...
		it("should reject storing result for task already in failed status", async () => {
			...
			await expect(
				store.storeTaskResult(taskId, "failed", secondResult),
			).rejects.toThrow("Cannot store result for task");
		});
		...
		it("should reject storing result for cancelled task", async () => {
			...
			await expect(
				store.storeTaskResult(taskId, "completed", result),
			).rejects.toThrow("Cannot store result for task");
		});
...
describe("InMemoryTaskMessageQueue", () => {
	...
	describe("queue size limits", () => {
		it("should throw when maxSize is exceeded", async () => {
			...
			await expect(
				queue.enqueue("task-limit", message, undefined, 2),
			).rejects.toThrow("Task message queue overflow");
		});
		...
		it("should allow enqueue when under maxSize", async () => {
			...
			await expect(
				queue.enqueue("task-ok", message, undefined, 5),
