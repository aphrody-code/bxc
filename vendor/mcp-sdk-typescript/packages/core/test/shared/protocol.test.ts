...
describe("protocol tests", () => {
	...
	describe("Debounced Notifications", () => {
		...
		it("should send non-debounced notifications immediately and multiple times", async () => {
			...
			protocol = new TestProtocolImpl({
				debouncedNotificationMethods: ["test/debounced"],
			}); // Configure for a different method
			await protocol.connect(transport);
...
describe("Task-based execution", () => {
	...
	describe("relatedTask metadata", () => {
		it("should inject relatedTask metadata into _meta field", async () => {
			...
			const queue = (protocol as unknown as TestProtocolInternals)._taskManager
				._taskMessageQueue;
		...
		it("should work with notification method", async () => {
			...
			expect(
				queuedMessage.message.params!._meta![RELATED_TASK_META_KEY],
	...
	describe("assertTaskHandlerCapability", () => {
		it("should invoke assertTaskHandlerCapability when an inbound task-augmented request arrives", async () => {
			...
			const spy = vi.spyOn(
				localProtocol,
				"assertTaskHandlerCapability" as never,
			);
			const localTransport = new MockTransport();
			await localProtocol.connect(localTransport);
		...
		it("should send a JSON-RPC error response when assertTaskHandlerCapability throws", async () => {
			...
			expect(response.error!.message).toBe(
				"Task handler capability not declared",
			);
		});
	});
	...
	describe("pollInterval fallback in _waitForTaskUpdate", () => {
		it("should fall back to defaultTaskPollInterval when task has no pollInterval", async () => {
			...
			setTimeout(() => {
				mockTaskStore.storeTaskResult(task.taskId, "completed", {
	...
	describe("listTasks", () => {
		...
		it("should call listTasks method from client side", async () => {
			...
			const listTasksPromise = (
				protocol as unknown as TestProtocolInternals
		...
		it("should call listTasks with cursor from client side", async () => {
			...
			const listTasksPromise = (
				protocol as unknown as TestProtocolInternals
			)._taskManager.listTasks({ cursor: "task-10" });
	...
	describe("cancelTask", () => {
		...
		it("should call cancelTask method from client side", async () => {
			...
			const deleteTaskPromise = (
				protocol as unknown as TestProtocolInternals
			)._taskManager.cancelTask({ taskId: "task-to-delete" });
	...
	describe("task metadata handling", () => {
		...
		it("should propagate related-task metadata to handler sendRequest and sendNotification", async () => {
			...
			const serverProtocol = createTestProtocol({
				taskStore: mockTaskStore,
				taskMessageQueue: new InMemoryTaskMessageQueue(),
			});
			...
			const queue = (serverProtocol as unknown as TestProtocolInternals)
				._taskManager._taskMessageQueue;
			expect(queue).toBeDefined();
			...
			const notificationCalls = sendSpy.mock.calls.filter(
				(call) =>
...
describe("Progress notification support for tasks", () => {
	...
	it("should stop progress notifications when task reaches terminal status (completed)", async () => {
		...
		const taskProgressTokens = (protocol as unknown as TestProtocolInternals)
			._taskManager._taskProgressTokens as Map<string, number>;
		...
		protocol.setRequestHandler("ping", async (_request, ctx) => {
			if (ctx.task?.store) {
				await ctx.task.store.storeTaskResult(taskId, "completed", {
					content: [],
				});
			}
			return {};
...
describe("Message interception for task-related notifications", () => {
	it("should queue notifications with io.modelcontextprotocol/related-task metadata", async () => {
		const taskStore = createMockTaskStore();
		const transport = new MockTransport();
		const server = createTestProtocol({
			taskStore,
			taskMessageQueue: new InMemoryTaskMessageQueue(),
		});
		...
		const queue = (server as unknown as TestProtocolInternals)._taskManager
			._taskMessageQueue;
		expect(queue).toBeDefined();
...
describe("Message interception for task-related requests", () => {
	it("should queue requests with io.modelcontextprotocol/related-task metadata", async () => {
		...
		const requestId = (queuedMessage!.message as JSONRPCRequest)
			.id as RequestId;
		const resolvers = (server as unknown as TestProtocolInternals)._taskManager
			._requestResolvers;
	...
	it("should log error when resolver is missing for side-channeled request", async () => {
		...
		(
			server as unknown as TestProtocolInternals
		)._taskManager._requestResolvers.delete(requestId);
...
describe("Message Interception", () => {
	...
	describe("messages with relatedTask metadata are queued", () => {
		...
		it("should queue requests with relatedTask metadata", async () => {
			...
			const resolvers = (protocol as unknown as TestProtocolInternals)
				._taskManager._requestResolvers;
			expect(resolvers.has(requestId)).toBe(true);
...
describe("Queue lifecycle management", () => {
	...
	describe("queue cleanup on task completion", () => {
		it("should clear queue when task reaches completed status", async () => {
			...
			await protocol.notification(
				{ method: "test/notification", params: { data: "test1" } },
				{ relatedTask: { taskId } },
			);
			await protocol.notification(
				{ method: "test/notification", params: { data: "test2" } },
				{ relatedTask: { taskId } },
			);
			...
			(
				protocol as unknown as TestProtocolInternals
			)._taskManager._clearTaskQueue(taskId);
		...
		it("should clear queue after delivering messages on tasks/result for completed task", async () => {
			...
			// Queue a message
			await protocol.notification(
				{ method: "test/notification", params: { data: "test" } },
				{ relatedTask: { taskId } },
			);
	...
	describe("resolver rejection on cleanup", () => {
		...
		it("should clean up resolver mappings when rejecting requests", async () => {
			...
			const requestResolvers = (protocol as unknown as TestProtocolInternals)
				._taskManager._requestResolvers;
	...
	describe("Error responses", () => {
		...
		test("should yield error as terminal message for timeout", async () => {
			...
			try {
				...
				expect((lastMessage.error as SdkError).code).toBe(
...
describe("Error handling for missing resolvers", () => {
	...
	describe("Response routing with missing resolvers", () => {
		it("should log error for unknown request ID without throwing", async () => {
			...
			if (queuedMessage && queuedMessage.type === "response") {
				...
				const resolver =
					testProtocol._taskManager._requestResolvers.get(requestId);
	...
	describe("Task cancellation with missing resolvers", () => {
		...
		it("should handle cleanup gracefully when resolver exists", async () => {
			...
			expect(testProtocol._taskManager._requestResolvers.has(requestId)).toBe(
				false,
			);
		});
	...
	describe("Error handling does not throw exceptions", () => {
		...
		it("should not throw during task cleanup with missing resolvers", async () => {
			...
			// This should not throw
			await expect(
				testProtocol._taskManager._clearTaskQueue(task.taskId),
