...
async function openBrowser(url: string): Promise<void> {
	try {
		...
		if (!ALLOWED_SCHEMES.has(parsed.protocol)) {
			console.error(
				`Refusing to open URL with unsupported scheme '${parsed.protocol}': ${url}`,
			);
			return;
		}
	} catch {
...
async function handleURLElicitation(
	...
	console.log(
		"\u001B[33mThe server is requesting you to open an external URL.\u001B[0m",
	);
	console.log(
		"\u001B[33mOnly proceed if you trust this server and understand why it needs this.\u001B[0m\n",
	);
	...
	const completionPromise = new Promise<void>((resolve, reject) => {
		const timeout = setTimeout(
			() => {
				...
				console.log(
					`\u001B[31m❌ Elicitation ${elicitationId} timed out waiting for completion.\u001B[0m`,
				);
	...
	client.setNotificationHandler(
		...
		(notification) => {
			...
			if (pending) {
				...
				pendingURLElicitations.delete(elicitationId);
				console.log(
					`\u001B[32m✅ Elicitation ${elicitationId} completed!\u001B[0m`,
				);
				pending.resolve();
			} else {
