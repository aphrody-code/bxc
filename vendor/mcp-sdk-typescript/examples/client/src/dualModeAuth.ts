...
function createHostManagedTransport(
	serverUrl: URL,
	session: HostSessionStore,
	ui: HostUi,
): StreamableHTTPClientTransport {
...
async function connectAndList(
	...
	const client = new Client(
		{ name: "dual-mode-example", version: "1.0.0" },
		{ capabilities: {} },
	);
...
async function main() {
	...
	if (mode === "host") {
		...
		const session: HostSessionStore = {
			getMcpToken: () => process.env.MCP_TOKEN,
		};
		const ui: HostUi = {
			showReauthPrompt: (msg) => console.error(`[UI] ${msg}`),
		};
		...
		if (!clientId || !clientSecret) {
			console.error(
				"OAUTH_CLIENT_ID and OAUTH_CLIENT_SECRET required for oauth mode",
			);
			process.exit(1);
		}
		transport = createUserConfiguredTransport(
			serverUrl,
			clientId,
			clientSecret,
		);
