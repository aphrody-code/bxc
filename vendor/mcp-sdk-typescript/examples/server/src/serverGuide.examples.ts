		...
		async ({ url }): Promise<CallToolResult> => {
			try {
				...
				if (!res.ok) {
					return {
						content: [
							{ type: "text", text: `HTTP ${res.status}: ${res.statusText}` },
						],
						isError: true,
					};
				}
		...
		"review-code",
		{
			...
			argsSchema: z.object({
				language: completable(
					...
					(value) =>
						["typescript", "javascript", "python", "rust", "go"].filter(
							(lang) => lang.startsWith(value),
						),
				),
			}),
		},
...
function registerTool_progress(server: McpServer) {
	...
	server.registerTool(
		...
		async ({ files }, ctx): Promise<CallToolResult> => {
			...
			return {
				content: [{ type: "text", text: `Processed ${files.length} files` }],
			};
		},
	);
...
function shutdown_statefulHttp(
