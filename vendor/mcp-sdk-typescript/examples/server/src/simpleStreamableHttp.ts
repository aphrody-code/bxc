...
const dangerousLoggingEnabled = process.argv.includes(
	"--dangerous-logging-enabled",
);
...
const getServer = () => {
	...
	server.experimental.tasks.registerToolTask(
		"collect-user-info-task",
		{
			...
			description:
				"Collects user info via elicitation with task support using elicitInputStream",
			inputSchema: z.object({
				infoType: z
				...
				(async () => {
					try {
						...
						const requestedSchema =
							infoType === "contact" ? contactSchema : preferencesSchema;
...
const AUTH_PORT = process.env.MCP_AUTH_PORT
	? Number.parseInt(process.env.MCP_AUTH_PORT, 10)
	: 3001;
...
app.listen(MCP_PORT, (error) => {
	...
	if (useOAuth) {
		console.log(
			`  Protected Resource Metadata: http://localhost:${MCP_PORT}/.well-known/oauth-protected-resource/mcp`,
		);
	}
});
