...
describe("Task Lifecycle Integration Tests", () => {
		...
		mcpServer.experimental.tasks.registerToolTask(
			"long-task",
			{
				...
				inputSchema: z.object({
					...
					shouldFail: z
						.boolean()
						.describe("Whether the task should fail")
						.default(false),
				}),
			},
			{
						...
						try {
							await (shouldFail
								? ctx.task.store.storeTaskResult(task.taskId, "failed", {
										content: [
											{ type: "text", text: "Task failed as requested" },
										],
										isError: true,
									})
								: ctx.task.store.storeTaskResult(task.taskId, "completed", {
										content: [
											{ type: "text", text: `Completed after ${duration}ms` },
										],
									}));
						} catch {
	...
	describe("Task Creation and Completion", () => {
		it("should create a task and return CreateTaskResult", async () => {
			...
			expect(result.content).toEqual([
				{ type: "text", text: "Completed after 500ms" },
			]);
	...
	describe("Error Handling", () => {
		it("should return error code -32602 for non-existent task in tasks/get", async () => {
			...
			await expect(
				client.experimental.tasks.getTask("non-existent-task-id"),
			).rejects.toSatisfy((error: ProtocolError) => {
				expect(error).toBeInstanceOf(ProtocolError);
				expect(error.code).toBe(ProtocolErrorCode.InvalidParams);
		...
		it("should return error code -32602 for non-existent task in tasks/cancel", async () => {
			...
			await expect(
				client.experimental.tasks.cancelTask("non-existent-task-id"),
			).rejects.toSatisfy((error: ProtocolError) => {
				expect(error).toBeInstanceOf(ProtocolError);
				expect(error.code).toBe(ProtocolErrorCode.InvalidParams);
				expect(error.message).toContain("Task not found");
			...
			mcpServer.experimental.tasks.registerToolTask(
				"cancellable-task",
				{
					...
					inputSchema: z.object({
						messageCount: z
							.number()
							.describe("Number of messages to queue")
							.default(2),
					}),
				},
				{
	...
	describe("Continuous Message Delivery", () => {
		it("should deliver messages immediately while tasks/result is blocking", async () => {
				...
				"streaming-task",
				{
					...
					inputSchema: z.object({
						messageCount: z
							...
							.default(3),
						delayBetweenMessages: z
							.number()
							.describe("Delay between messages in ms")
							.default(200),
					}),
				},
				{
						...
						(async () => {
							try {
								...
								for (let i = 0; i < messageCount; i++) {
									...
									if (i < messageCount - 1) {
										await Bun.sleep(delayBetweenMessages);
									}
								}
									...
									await ctx.task.store.storeTaskResult(
										...
										"completed",
										{
											content: [
												{
													type: "text",
													text: `Received all responses: ${responses.join(", ")}`,
												},
											],
										},
									);
			...
			const timeBetweenSecondAndThird =
				receivedMessages[2]!.timestamp - receivedMessages[1]!.timestamp;
							...
							try {
								...
								try {
									await ctx.task.store.storeTaskResult(
										...
										{
											content: [
												{ type: "text", text: "Task completed quickly" },
											],
										},
									);
								} catch {
	...
	describe("callToolStream with elicitation", () => {
		it("should deliver elicitation via callToolStream and complete task", async () => {
			...
			expect(result.content).toEqual([
				{ type: "text", text: "Hello, StreamUser!" },
			]);
