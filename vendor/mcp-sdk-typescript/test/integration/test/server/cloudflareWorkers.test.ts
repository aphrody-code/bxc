...
describe('Cloudflare Workers compatibility (no nodejs_compat)', () => {
    ...
    beforeAll(async () => {
        ...
        await Bun.write(path.join(tempDir, 'package.json'), JSON.stringify(pkgJson, null, 2));
        ...
        await Bun.write(path.join(tempDir, 'wrangler.jsonc'), JSON.stringify(wranglerConfig, null, 2));
...
`;
        await Bun.write(path.join(tempDir, 'server.ts'), serverSource);
