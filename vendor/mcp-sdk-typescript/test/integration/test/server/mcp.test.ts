...
describe("Zod v4", () => {
	describe("McpServer", () => {
		...
		test("should request sampling via ctx.mcpReq.requestSampling() convenience method", async () => {
			...
			mcpServer.registerTool(
				...
				{
					description:
						"A tool that requests sampling via ctx.mcpReq.requestSampling()",
				},
				async (ctx) => {
		...
		test("should register and advertise server extensions capability", async () => {
			...
			expect(
				capabilities?.extensions?.["io.modelcontextprotocol/test-extension"],
			...
			mcpServer.experimental.tasks.registerToolTask(
				...
				},
				{
					...
					getTaskResult: async (_args, ctx) => {
						return (await ctx.task.store.getTaskResult(
							ctx.task.id,
						)) as CallToolResult;
					},
				},
	...
	describe("resource()", () => {
		...
		test("should remove resource and send notification when connected", async () => {
			...
			const resource1 = mcpServer.registerResource(
				"resource1",
				"test://resource1",
				{},
				async () => ({
	...
	describe("prompt()", () => {
		...
		test("should update prompt with schema", async () => {
			...
			expect(
				listResult.prompts[0]!.arguments!.map((a) => a.name).toSorted(),
			).toEqual(["name", "value"]);
		...
		test("should remove prompt and send notification when connected", async () => {
			...
			expect(result.prompts.map((p) => p.name).toSorted()).toEqual([
				"prompt1",
				"prompt2",
			]);
				...
				async ({ restaurant, date, partySize }) => {
					...
					if (!available) {
						...
						if (
							...
							result.content?.checkAlternatives
						) {
		...
		test("should support z.transform() schemas", async () => {
			...
			server.registerTool(
				...
				async (args) => {
					return {
						content: [
							{ type: "text" as const, text: `Full name: ${args.fullName}` },
						],
					};
				},
			);
			...
			server.registerTool(
				...
				async (args) => {
					return args.type === "email"
						? {
								content: [
									{ type: "text", text: `Email contact: ${args.email}` },
								],
							}
						: {
								content: [
									{ type: "text", text: `Phone contact: ${args.phone}` },
								],
							};
				},
			);
	...
	describe("Tool-level task hints with automatic polling wrapper", () => {
					...
					createTask: async ({ input }, ctx) => {
						...
						setTimeout(async () => {
							await store.storeTaskResult(task.taskId, "completed", {
								content: [
									{ type: "text" as const, text: `Processed: ${input}` },
								],
							});
						}, 200);
		...
		test('should automatically poll and return CallToolResult for tool with taskSupport "optional" called without task augmentation', async () => {
				...
				},
				{
					createTask: async ({ value }, ctx) => {
						...
						setTimeout(async () => {
							await store.storeTaskResult(task.taskId, "completed", {
								content: [
									{ type: "text" as const, text: `Result: ${value * 2}` },
								],
							});
							releaseLatch();
						}, 150);
			...
			expect(result.content).toEqual([
				{ type: "text" as const, text: "Result: 42" },
			]);
				...
				},
				{
					createTask: async ({ data }, ctx) => {
						...
						setTimeout(async () => {
							await store.storeTaskResult(task.taskId, "completed", {
								content: [
									{ type: "text" as const, text: `Completed: ${data}` },
								],
							});
							releaseLatch();
						}, 200);
