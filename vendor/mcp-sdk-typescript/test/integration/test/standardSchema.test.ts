...
import {
	completable,
	fromJsonSchema as serverFromJsonSchema,
	McpServer,
		...
		describe("tool registration", () => {
			test("should register tool with ArkType input schema", async () => {
				...
				mcpServer.registerTool(
					...
					async ({ name, age }) => ({
						content: [
							{ type: "text", text: `Hello ${name}, you are ${age} years old` },
						],
					}),
				);
			...
			test("should register tool with ArkType input and output schemas", async () => {
				...
				expect(result.tools[0].outputSchema!.required).toEqual(
					expect.arrayContaining(["result", "operation"]),
				);
			});
		});
	...
	describe("Mixed schema libraries", () => {
		test("should support tools with different schema libraries in same server", async () => {
			...
			mcpServer.registerTool(
				...
				async ({ value }) => ({
					content: [{ type: "text", text: `valibot: ${value}` }],
				}),
			);
			...
			expect((arktypeResult.content[0] as TextContent).text).toBe(
				"arktype: test",
			);
			...
			expect((valibotResult.content[0] as TextContent).text).toBe(
				"valibot: test",
			);
		});
	});
			...
			mcpServer.registerPrompt(
				...
				async ({ name }) => ({
					messages: [
						{
							role: "user",
							content: { type: "text", text: `Hello ${name ?? "there"}` },
						},
					],
				}),
			);
				...
				async ({ name, count, enabled }) => {
					...
					return {
						content: [
							{
								type: "text",
								text: `${_name}: ${_count}, enabled: ${_enabled}`,
							},
						],
					};
				},
