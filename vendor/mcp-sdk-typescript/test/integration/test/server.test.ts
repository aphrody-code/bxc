...
test("should wrap unexpected validator errors during elicitInput", async () => {
	...
	await expect(
		...
		"Error validating elicitation response: boom - validator exploded",
	);
});
...
test("should create notifier that emits elicitation completion notification", async () => {
	...
	const notifier =
		server.createElicitationCompletionNotifier("elicitation-123");
	await notifier();
...
describe("createMessageStream", () => {
	...
	describe("with tasks", () => {
		let server: Server;
		let client: Client;
		let clientTransport: ReturnType<
			typeof InMemoryTransport.createLinkedPair
		>[0];
		let serverTransport: ReturnType<
			typeof InMemoryTransport.createLinkedPair
		>[1];
		...
		describe("terminal message guarantees", () => {
			test("should yield exactly one terminal message for successful request", async () => {
				...
				const taskMessages = allMessages.filter(
					(m) => m.type === "taskCreated" || m.type === "taskStatus",
				);
			...
			test("should yield exactly one terminal message with result", async () => {
				...
				if (lastMessage.type === "result") {
					expect(
						(lastMessage.result as CreateMessageResult).content,
					).toBeDefined();
				}
			});
		});
		...
		describe("task-augmented request handling", () => {
			test("should yield taskCreated and result for task-augmented request", async () => {
				...
				const [taskClientTransport, taskServerTransport] =
					InMemoryTransport.createLinkedPair();
				await Promise.all([
					taskClient.connect(taskClientTransport),
					server.connect(taskServerTransport),
				]);
				...
				const stream = server.experimental.tasks.createMessageStream(
					{
						messages: [
							{
								role: "user",
								content: { type: "text", text: "Task-augmented message" },
							},
						],
						maxTokens: 100,
					},
				...
				if (lastMessage.type === "result") {
					expect((lastMessage.result as CreateMessageResult).model).toBe(
						"test-model",
					);
				}
...
describe("createMcpExpressApp", () => {
	...
	test("should use custom allowedHosts when provided", async () => {
		...
		const allowedResponse = await supertest(app)
			.post("/test")
			.set("Host", "myapp.local:3000")
			.send({});
		expect(allowedResponse.status).toBe(200);

		// Should reject other hosts
		const rejectedResponse = await supertest(app)
			.post("/test")
			.set("Host", "evil.com:3000")
			.send({});
...
describe("Task-based execution", () => {
	...
	test("server without TaskStore should reject task-based requests", async () => {
		...
		await expect(
			client.experimental.tasks.getTask("non-existent"),
	...
	test("should automatically attach related-task metadata to nested requests during tool execution", async () => {
		...
		let capturedElicitRequest: z.infer<typeof ElicitRequestSchema> | null =
			null;
	...
	test("should handle multiple concurrent task-based tool calls", async () => {
		...
		for (const [i, taskId] of taskIds.entries()) {
			...
			expect(result.content).toEqual([
				{ type: "text", text: `Completed task ${i + 1}` },
			]);
		}
	...
	describe("Error scenarios", () => {
		...
		test("should throw error when client queries non-existent task from server", async () => {
			...
			await expect(
				client.experimental.tasks.getTask("non-existent-task"),
			).rejects.toThrow();
		});
		...
		test("should throw error when server queries non-existent task from client", async () => {
			...
			await expect(
				server.experimental.tasks.getTask("non-existent-task"),
			).rejects.toThrow();
		});
	});
});
...
test("should respect client task capabilities", async () => {
	...
	await expect(
		server.experimental.tasks.getTask(taskId),
	).resolves.not.toThrow();
...
describe("elicitInputStream", () => {
	...
	test("should throw when client does not support form elicitation", async () => {
		...
		const [noFormClientTransport, noFormServerTransport] =
			InMemoryTransport.createLinkedPair();

		await Promise.all([
			noFormClient.connect(noFormClientTransport),
			server.connect(noFormServerTransport),
		]);
	...
	test("should throw when client does not support url elicitation", async () => {
		...
		const [noUrlClientTransport, noUrlServerTransport] =
			InMemoryTransport.createLinkedPair();

		await Promise.all([
			noUrlClient.connect(noUrlClientTransport),
			server.connect(noUrlServerTransport),
		]);
	...
	test("should default to form mode when mode is not specified", async () => {
		const requestStreamSpy = vi.spyOn(
		...
		const stream = server.experimental.tasks.elicitInputStream(
			params as Parameters<
				typeof server.experimental.tasks.elicitInputStream
			>[0],
		);
	...
	describe("terminal message guarantees", () => {
		...
		])("should yield exactly one terminal message for action: $action", async ({
			action,
			content,
		}) => {
			client.setRequestHandler("elicitation/create", () => ({
	...
	describe("non-task request minimality", () => {
		test.each([
			...
			{ action: "decline" as const, content: undefined },
			{ action: "cancel" as const, content: undefined },
		])("should yield only result message for non-task request with action: $action", async ({
			action,
			content,
		}) => {
			client.setRequestHandler("elicitation/create", () => ({
	...
	describe("task-augmented request handling", () => {
		test("should yield taskCreated and result for task-augmented request", async () => {
			...
			if (lastMessage.type === "result") {
				...
				expect((lastMessage.result as ElicitResult).content).toEqual({
					username: "task-user",
				});
			}
