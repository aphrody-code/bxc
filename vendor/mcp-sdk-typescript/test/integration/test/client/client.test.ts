...
test("should restore negotiated protocol version on transport when reconnecting with same client", async () => {
	...
	expect(reconnectTransport.send).not.toHaveBeenCalledWith(
		expect.objectContaining({ method: "initialize" }),
		expect.anything(),
	);
	// But the protocol version MUST have been restored onto the new transport
	expect(reconnectSetProtocolVersion).toHaveBeenCalledWith(
		LATEST_PROTOCOL_VERSION,
	);
});
	...
	test("should throw error when tool with outputSchema returns no structuredContent", async () => {
		...
		server.setRequestHandler("tools/call", async (request) => {
			if (request.params.name === "test-tool") {
				...
				return {
					content: [
						{ type: "text", text: "This should be structured content" },
					],
				};
			}
			throw new Error("Unknown tool");
	...
	describe("Client calling server", () => {
		...
		test("should query task list from server using listTasks", async () => {
			...
			for (let i = 0; i < 2; i++) {
				...
				const taskList = await client.experimental.tasks.listTasks();
				const newTask = taskList.tasks.find(
					(t) => !createdTaskIds.includes(t.taskId),
				);
				if (newTask) {
...
describe("Client sampling validation with tools", () => {
	test("should validate array content with tool_use when request includes tools", async () => {
		...
		expect((result.content as Array<{ type: string }>)[0]!.type).toBe(
			"tool_use",
		);
	});
