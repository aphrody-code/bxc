...
import { isGoogleDomain } from "../google/dns.ts";
...
export class RateLimiter {
	...
	async acquire(url: string): Promise<void> {
		...
		let maxSec = this.#maxPerSecond;
		let maxMin = this.#maxPerMinute;

		// Google is extremely sensitive to rate, auto-throttle if it's a google domain
		if (isGoogleDomain(host)) {
			maxSec = Math.min(maxSec, 1); // Max 1 req/sec for Google
			maxMin = Math.min(maxMin, 30); // Max 30 req/min
		}

		await this.#enforceSliding(host, maxSec, maxMin);
	 ...
	 */
	async #enforceSliding(host: string, maxPerSecond: number, maxPerMinute: number): Promise<void> {
		const state = this.#getOrCreateState(host);
		...
		while (true) {
			...
			const secondFull = state.secondWindow.length >= maxPerSecond;
			const minuteFull = state.minuteWindow.length >= maxPerMinute;
			...
			if (secondFull && minuteFull) {
				const secondWait = waitUntilExpiry(state.secondWindow, now, 1_000, maxPerSecond);
				const minuteWait = waitUntilExpiry(state.minuteWindow, now, 60_000, maxPerMinute);
				sleepMs = Math.min(secondWait, minuteWait);
			} else if (secondFull) {
				sleepMs = waitUntilExpiry(state.secondWindow, now, 1_000, maxPerSecond);
			} else {
				sleepMs = waitUntilExpiry(state.minuteWindow, now, 60_000, maxPerMinute);
			}
