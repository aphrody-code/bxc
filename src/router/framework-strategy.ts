 ...
 * Algorithm :
 *
 *  1. If a known anti-bot WAF is detected → escalate to `stealth` (and `max`
 *     for the most aggressive vendors like Datadome / Akamai / Kasada).
 *  2. Else if an SPA framework is detected without a known SSR companion →
 *     `fast` profile + wait for hydration.
 *  3. Else if an SSR framework is detected → `fast` with `domcontentloaded`.
 *  4. Else if a static-friendly CMS is detected → `static` profile.
 *  5. Else → default to `static` (cheapest), but suggest a re-detect.
 */
export function suggestStrategy(detected: DetectedTech[]): Strategy {
