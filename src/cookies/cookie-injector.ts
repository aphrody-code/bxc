...
/**
 * Internal: send a single CDP command on a transport (no session id) and
...
export async function injectCookies(
	...
	try {
		await cdpCall(transport, "Network.setCookies", { cookies: cdpCookies }, sessionId);
		return;
	} catch (err) {
 ...
 * ```
 */
export function buildCookieHeader(cookies: Cookie[], url: string): string | null {
