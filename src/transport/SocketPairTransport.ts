 ...
 */
async function waitForReady(host: string, port: number, timeoutMs: number): Promise<string> {
...
export class SocketPairTransport implements ConnectionTransport {
	 ...
	 */
	static async create(opts: SocketPairTransportOptions = {}): Promise<SocketPairTransport> {
		...
		const args = ["serve", "--host", host, "--port", String(port), "--log-level", logLevel];
		...
		drain(proc.stdout as ReadableStream<Uint8Array> | undefined);
		drain(proc.stderr as ReadableStream<Uint8Array> | undefined, opts.stderrLogger);

		// Race readiness against process exit
		let earlyExit: Error | null = null;
		const exitPromise = proc.exited.then((code) => {
			earlyExit = new Error(`lightpanda exited prematurely (code ${code}) before becoming ready`);
		});
	...
	#watchProcess(): void {
		...
		void this.#proc.exited.then(async (code) => {
			...
			if (this.#opts.autoRespawn && this.#respawnCount < (this.#opts.maxRespawns ?? 3)) {
				this.#respawnCount++;
				try {
					await this.#respawn();
				} catch {
