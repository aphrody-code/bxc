...
class ParsedDocument {
	...
	#parseAttrsRegex(openingTag: string): Record<string, string> {
		const attrs: Record<string, string> = {};
		const re = /(\w[\w-]*)(?:\s*=\s*(?:"([^"]*)"|'([^']*)'|(\S+)))?/g;
		let m: RegExpExecArray | null;
		while ((m = re.exec(openingTag)) !== null) {
			const name = m[1];
			if (name && name.toLowerCase() !== openingTag.trimStart().split(/\s/)[0].replace("<", "")) {
...
const NOT_SUPPORTED_MSG = "not supported in http profile (HTTP-only, no JS engine, no input layer)";
...
export class HttpProfileTransport implements ConnectionTransport {
	 ...
	 */
	static async create(opts: ImpersonatedClientOptions = {}): Promise<HttpProfileTransport> {
	...
	send(message: string): void {
		...
		this.#handle(req).then(
			(result) => {
				this.#dispatch({ id: req.id, result, sessionId: req.sessionId } satisfies CDPResponse);
			},
			(err: unknown) => {
				const error =
				...
				this.#dispatch({ id: req.id, error, sessionId: req.sessionId } satisfies CDPResponse);
			},
		);
	}
	...
	async #handle(req: CDPRequest): Promise<unknown> {
		...
		switch (method) {
			...
			case "Target.getTargetInfo": {
				const { targetId } = params as { targetId?: string };
				const page = targetId ? this.#pages.get(targetId) : this.#firstOrCreate();
			...
			case "DOM.describeNode": {
				const { nodeId } = params as { nodeId?: number };
				const page = this.#pageBySession(sessionId);
				const node = nodeId !== undefined ? page.doc?.getNodeById(nodeId) : undefined;
				return {
					node: {
						nodeId: nodeId ?? 0,
	...
	#firstOrCreate(): HttpPageState {
		const first = this.#pages.values().next().value as HttpPageState | undefined;
