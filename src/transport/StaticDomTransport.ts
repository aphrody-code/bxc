...
import { isZigQueryAvailable, parseHtml as zigParseHtml, type ZigDoc } from "../ffi/zigquery.js";
 ...
 */
const zigDocFinalizer = new FinalizationRegistry<ZigDocCleanupToken>((token) => {
	try {
		token.doc.destroy();
	} catch {
...
class ParsedDocument implements ParsedDocumentLike {
	...
	constructor(html: string, url = "about:blank") {
		this.rawHtml = html;
		this.url = url;
		this.title = ParsedDocument.#extractTitle(html);
		this.rootId = this.#allocId();
		this.#nodes.set(this.rootId, {
			nodeId: this.rootId,
			tagName: "#document",
			outerHTML: html,
			textContent: ParsedDocument.#stripTags(html),
			attributes: {},
		});
	...
	querySelectorAll(selector: string): ParsedNode[] {
		...
		if (zigDoc) {
			...
			for (let i = 0; i < sel.count; i++) {
				...
				// We only need the opening tag — stop at the first ">".
				const openingTag = outerHTML.slice(0, outerHTML.indexOf(">") + 1);
				const node: ParsedNode = {
					nodeId: id,
					tagName: el.tagName().toLowerCase(),
					outerHTML,
					textContent: el.textContent(),
					attributes: this.#parseAttrsRegex(openingTag),
				};
		...
		// Regex fallback
		return this.#scanElementsRegex().filter((el) => this.#matchesRegex(el, selector));
	}
	...
	toCDPNode(node: ParsedNodeLike): InternalDOMNode {
		return {
			...
			nodeName: node.tagName === "#document" ? "#document" : node.tagName.toUpperCase(),
	...
	#scanElementsRegex(): ParsedNode[] {
		...
		const TAG_RE = /<([a-zA-Z][a-zA-Z0-9-]*)(\s[^>]*)?>[\s\S]*?(?=<[a-zA-Z]|$)/g;
		let m: RegExpExecArray | null;
		while ((m = TAG_RE.exec(this.rawHtml)) !== null) {
			const tagName = m[1].toLowerCase();
			const attrStr = m[2] ?? "";
			const attrs = this.#parseAttrsRegex(attrStr);
	...
	#parseAttrsRegex(attrStr: string): Record<string, string> {
...
class StaticDomHandler {
	...
	async #navigate(page: PageState, url: string): Promise<void> {
		...
		const userAgent = page.emulation.userAgent ?? "Bunlight/0.1.0 StaticDomTransport";
		...
		if (fetchState?.enabled && fetchState.patterns.length > 0) {
			...
			if (matched) {
				...
				this.#emitEvent({
					...
					params: {
						requestId: pauseRequestId,
						request: { url, method: "GET", headers, initialPriority: "VeryHigh" },
						frameId: page.frameId,
						resourceType: "Document",
						networkId: requestId,
					},
		...
		try {
			...
			const mimeType = (responseHeaders["content-type"] ?? "text/html").split(";")[0].trim();
 ...
 */
export class StaticDomTransport extends InProcessTransport implements ConnectionTransport {
