...
import { dlopen, FFIType, JSCallback, toArrayBuffer, type Library, ptr } from "bun:ffi";
...
function resolveLibPath(): string {
	...
	// Walk up to find vendor/curl-impersonate/
	const candidates = [
		join(dir, "../../vendor/curl-impersonate/libcurl-impersonate.so.4.8.0"),
		join(dir, "../../vendor/curl-impersonate/libcurl-impersonate.so.4"),
		join(dir, "../../vendor/curl-impersonate/libcurl-impersonate.so"),
		// Allow override via env
		process.env.LIBCURL_IMPERSONATE_PATH ?? "",
	].filter(Boolean);

	// Return the first candidate that exists on disk, using Bun.spawnSync for a
	// synchronous existence check (Bun.file().exists() is async-only).
	for (const c of candidates) {
		const result = Bun.spawnSync(["test", "-f", c], { stdin: "ignore" });
		if (result.exitCode === 0) return c;
	}
	throw new Error(
		`libcurl-impersonate not found. Expected at vendor/curl-impersonate/libcurl-impersonate.so.4.8.0. ` +
...
function setoptStr(
	...
	const code = sym.curl_easy_setopt(handle, opt, BigInt(ptr(buf)) as unknown as number);
	if (code !== CURLE_OK) {
...
export class ImpersonatedClient {
	 ...
	 */
	async fetch(url: string, opts: FetchOptions = {}): Promise<ImpersonatedResponse> {
		...
		if (opts.body !== undefined) {
			...
			if (code !== CURLE_OK) throw new CurlError("CURLOPT_COPYPOSTFIELDS failed", code);
		}
		...
		if (headersToSet.length > 0) {
			...
			const code = sym.curl_easy_setopt(handle, CURLOPT.HTTPHEADER, slistPtr as unknown as number);
			if (code !== CURLE_OK) throw new CurlError("CURLOPT_HTTPHEADER failed", code);
		}
		...
		try {
			const buf = encodeC("");
			alive.push(buf);
			sym.curl_easy_setopt(handle, CURLOPT.ACCEPT_ENCODING, BigInt(ptr(buf)) as unknown as number);
		} catch {
		...
		setoptLong(sym, handle, CURLOPT.CONNECTTIMEOUT_MS, BigInt(Math.min(10_000, timeoutMs)));
		...
		// SSL
		setoptLong(sym, handle, CURLOPT.SSL_VERIFYPEER, this.#opts.sslVerify ? 1n : 0n);
		setoptLong(sym, handle, CURLOPT.SSL_VERIFYHOST, this.#opts.sslVerify ? 2n : 0n);
		...
		try {
			...
			sym.curl_easy_setopt(handle, CURLOPT.WRITEFUNCTION, BigInt(writeFnPtr) as unknown as number);
			...
			if (code !== CURLE_OK) {
				...
				throw new CurlError(`curl_easy_perform failed: ${errMsg} — ${url}`, code);
			}
	 ...
	 */
	async postJSON<T = unknown>(url: string, data: unknown, opts: FetchOptions = {}): Promise<T> {
...
export function getDefaultClient(): ImpersonatedClient {
	if (!_defaultClient || (_defaultClient as unknown as { _closed: boolean })._closed) {
