...
import { injectCookies, buildCookieHeader } from "../cookies/cookie-injector.ts";
...
export class Page implements AsyncDisposable {
	...
	constructor(transport: ConnectionTransport, targetId: string, sessionId: string) {
	...
	/** @internal */
	static async create(transport: ConnectionTransport, opts: PageOptions = {}): Promise<Page> {
		...
		const attached = (await Page.#cdpDirect(transport, "Target.attachToTarget", {
			targetId: result.targetId,
			flatten: true,
		})) as { sessionId: string };

		const page = new Page(transport, result.targetId, attached.sessionId);
	...
	async content(): Promise<string> {
		...
		const { outerHTML } = (await this.#send("DOM.getOuterHTML", {
	...
	> {
		...
		const result = (await this.#send("Network.getCookies", params)) as {
			cookies: Array<Record<string, unknown>>;
		};
		return result.cookies as ReturnType<Page["getCookies"]> extends Promise<infer T> ? T : never;
	}
	...
	async clearCookies(filter?: {
		...
		if (filter.name) {
			await this.#send("Network.deleteCookies", filter);
			return;
		}
		...
		for (const c of all) {
			...
			await this.#send("Network.deleteCookies", { name: c.name, domain: c.domain });
		}
	}
	...
	async unroute(pattern?: string | RegExp): Promise<void> {
		...
		if (this.#routes.length === 0 && this.#interceptionEnabled) {
			...
			await this.#send("Network.setRequestInterception", { patterns: [] }).catch(() => undefined);
		}
	}
	...
	async blockResources(families: ResourceFamily[]): Promise<void> {
		...
		await this.route(/.*/i, async (route) => {
			...
			if (blocked.has(family) || blocked.has(family.replace(/stylesheet/, "stylesheet"))) {
				await route.abort();
			} else {
	...
	#send(method: string, params: Record<string, unknown>): Promise<unknown> {
		return new Promise<unknown>((resolve, reject) => {
			...
			this.#transport.send(JSON.stringify({ id, method, params, sessionId: this.#sessionId }));
		});
	}
	...
	#handleMessage(raw: string): void {
		...
		if (msg.error) {
			pending.reject(new Error(`CDP ${msg.method ?? "?"} error: ${msg.error.message}`));
		} else {
	...
	async #enableInterception(): Promise<void> {
		...
		await this.#send("Network.setRequestInterception", {
			patterns: [{ urlPattern: "*" }],
		}).catch((err: Error) => {
	...
	async #dispatchInterception(params: Record<string, unknown>): Promise<void> {
		...
		const route: InterceptedRoute = {
			...
			continue: async () => {
				if (resolved) return;
				resolved = true;
				await this.#send("Network.continueInterceptedRequest", {
					interceptionId,
				}).catch(() => undefined);
			},
 ...
 */
async function resolveCookies(input: string | Cookie[] | undefined): Promise<Cookie[]> {
	if (!input) return [];
...
class BrowserSingleton {
	 ...
	 */
	async newPage(opts: PageOptions = {}): Promise<Page | HttpPage> {
		...
		if (profile === "http") {
			...
			const curlClient = new ImpersonatedClient(opts.httpOpts ?? { profile: "chrome131" });
			return new HttpPage(curlClient, cookies);
		}

		if (profile === "fast") {
			// Lazily import to avoid loading Bun.spawn-dependent code in static-only
			// environments (e.g., edge runtimes).
			const { SocketPairTransport } = await import("../transport/SocketPairTransport.js");
			...
			const page = await Page.create(fullTransport, opts);
			this.#pages.push(page);
			this.#fastTransports.set(page, fullTransport);

			if (cookies.length > 0) {
				await injectCookies(page._internalTransport, cookies, page._internalSessionId).catch(
					() => undefined,
				);
			}
		...
		const page = await Page.create(t, opts);
		this.#pages.push(page);
	...
	async close(): Promise<void> {
		...
		await Promise.all(
			Array.from(this.#fastTransports.values()).map((t) => t.closeProcess().catch(() => undefined)),
		);
...
export const Browser: {
	 ...
	 */
	newPage(opts?: PageOptions): Promise<Page | HttpPage>;
