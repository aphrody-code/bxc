...
import { launchGhostBrowser, type GhostBrowser } from "../profiles/ghost/index.ts";
...
export class XComScraper {
   ...
   */
  async init(_options?: { headless?: boolean }) {
   ...
   */
  async extractProfile(username: string, withScreenshot = false): Promise<XComProfileData> {
    if (!this.ghost) {
    ...
    if (!response.ok && response.status !== 0) {
      ...
      console.warn(`[XComScraper] Navigation might have failed with status ${response.status}`);
    }
