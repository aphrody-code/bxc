	...
	for (let i = 0; i < args.length; i++) {
		...
		switch (a) {
			...
			case "--profile": {
				...
				profile = v as CLIOptions["profile"];
				break;
			}
			case "--proxy":
				proxy = next();
...
function startStatic(opts: CLIOptions, logger: Logger): Server<WSData> {
...
function startHttp(opts: CLIOptions, logger: Logger): Server<WSData> {
	const { cdpPort, host } = opts;
...
async function main(): Promise<void> {
	...
	switch (opts.profile) {
		...
		case "http":
			...
			break;
		case "stealth":
			await startStealth(opts, logger);
			break;
		case "max":
			await startMax(opts, logger);
			break;
	}
