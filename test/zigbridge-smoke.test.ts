...
import { dlopen, FFIType, suffix, ptr, read, CString } from "bun:ffi";
...
// ---------------------------------------------------------------------------
// Helper: encode a string to a pinned Uint8Array for FFI passing
// ---------------------------------------------------------------------------
...
let lib: Lib;
const { symbols } = (() => {
	// We defer opening to beforeAll; declare symbols type here.
	return { symbols: null as unknown as Lib["symbols"] };
})();
...
describe("zigbridge smoke test — liblightpanda_dom", () => {
	...
	test("parse HTML document — returns non-null handle", () => {
		...
		const doc = _symbols.bl_doc_from_html(ptr(html), html.byteLength);
	...
	test("find h1 selector — count == 1", () => {
		...
		const sel = _symbols.bl_doc_find(doc!, ptr(selStr), selStr.byteLength);
	...
	test("bl_sel_text_into returns 'Hi'", () => {
		...
		const el = _symbols.bl_sel_at(sel!, 0);
	...
	test("bl_sel_attr_into returns id value", () => {
		...
		const out = new Uint8Array(BL_STRING_SIZE);
		_symbols.bl_sel_attr_into(el, ptr(attrName), attrName.byteLength, ptr(out));
	...
	test("empty selector result — count == 0, bl_sel_at returns null", () => {
		...
		expect(_symbols.bl_sel_at(sel, 0)).toBeNull();
