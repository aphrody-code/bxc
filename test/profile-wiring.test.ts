...
const SERVE_SCRIPT = "/home/ubuntu/vps/packages/bunlight/src/cli/serve.ts";
const BUNLIGHT_DIR = "/home/ubuntu/vps/packages/bunlight";
...
async function readPortFromOutput(
			...
			while (Date.now() < deadline) {
				...
				const raceResult = await Promise.race([
					...
					new Promise<{ done: true; value: undefined }>((_, reject) =>
						setTimeout(() => reject(new Error("timeout")), Math.max(timeout, 100)),
					),
				]);
	...
	const [stderrPort] = await Promise.race([
		...
		scanStream(proc.stderr as ReadableStream<Uint8Array> | null, "stderr").then((p) => [p]),
		// Then stdout
		scanStream(proc.stdout as ReadableStream<Uint8Array> | null, "stdout").then((p) => [p]),
		// Hard timeout
...
async function getBrowserVersion(wsUrl: string): Promise<{ product?: string }> {
	...
	const reply = await new Promise<string>((resolve, reject) => {
		const timer = setTimeout(() => reject(new Error("WS reply timeout")), 10_000);
	...
	ws.close();
	return JSON.parse(reply) as { result: { product?: string } } & { product?: string };
}
...
async function runProfileBootTest(profile: string): Promise<void> {
	...
	try {
		...
		const versionResp = await getBrowserVersion(body.webSocketDebuggerUrl as string);
		const result = (versionResp as unknown as { result?: { product?: string } }).result;
...
test(
	...
	async () => {
		...
		const curlVendorDir = "/home/ubuntu/vps/packages/bunlight/vendor/curl-impersonate";
