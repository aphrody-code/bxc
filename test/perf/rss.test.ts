...
const STATIC_DOM_TRANSPORT = "/home/ubuntu/vps/packages/bunlight/src/transport/StaticDomTransport.ts";
...
async function cdpNavigate(port: number, url: string): Promise<void> {
	return new Promise<void>((resolve) => {
		...
		ws.addEventListener("message", (ev: MessageEvent) => {
			...
			let msg: Msg;
			try {
				msg = JSON.parse(typeof ev.data === "string" ? ev.data : String(ev.data)) as Msg;
			} catch {
				return;
			}
			...
			if (msg.id === 2 && msg.result) {
				const sessionId = (msg.result["sessionId"] as string | undefined) ?? null;
				if (sessionId) {
					ws.send(JSON.stringify({ id: 10, sessionId, method: "Page.navigate", params: { url } }));
				}
				return;
			}
			if (msg.id === 10) {
...
describe("RSS memory targets", () => {
	test(
		...
		async () => {
			...
			const proc = Bun.spawn(
				...
				{ stdin: "ignore", stdout: "ignore", stderr: "ignore", cwd: BUNLIGHT_DIR },
			);
			...
			try {
				...
				console.log(`[rss.test] profile=static idle RSS = ${idleRss.toFixed(1)} MB`);
	...
	test("profiles/index.ts exports lazy loader functions", async () => {
		const { loadStaticProfile, loadHttpProfile, loadFastProfile, loadGhostProfile } = await import(
