 ...
 *   - Escalate to "stealth" (patchright) or "max" (Camoufox + CapSolver) if challenge detected
...
for (const url of urls) {
	...
	try {
		...
		await using stealthPage = await openStealthBrowser({
			blockResources: ["image", "font", "media"],
			timeoutMs: 30_000,
			headless: true,
		});

		await stealthPage.goto(url, { waitUntil: "domcontentloaded" });
		const content = await stealthPage.content();
		...
		if (detection.recommendation === "max") {
			...
			const maxTitle = await maxPage.title();
			const elapsedMs = performance.now() - start;
			results.push({ url, ok: true, title: maxTitle, profile, elapsedMs });
			console.log(
				`[${results.length}/${urls.length}] max profile | captcha.detected=${captcha.detected} | "${maxTitle}" (${elapsedMs.toFixed(0)}ms)`,
			);
		} else {
			profile = "stealth";
