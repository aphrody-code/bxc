...
const OUT_DIR = join(BUNLIGHT_DIR, "out/chromium-developers");
...
async function crawl(): Promise<Map<string, ExtractedPage>> {
	...
	const robots = await RobotsFile.fetch(SEED, { userAgent: "Bunlight/1.0" });
	const crawlDelay = robots.crawlDelay("Bunlight") ?? robots.crawlDelay("*");
	...
	async function processOne(req: { id: number; url: string }): Promise<void> {
		...
		if (!robots.isAllowed(url, "Bunlight")) {
