...
### Code source `src/` (14 500+ lignes TS)
- `api/browser.ts` — Browser singleton + Page / HttpPage unifiées via interface commune, support de `upgradeProfile()`.
- `api/BrowserContext.ts` (Nouveau) — Isolation totale des sessions (cookies, pages), parité Playwright.
- `api/Frame.ts` (Nouveau) — Support hiérarchique du multi-frame (iframes), parité Playwright.
- `api/Locator.ts` (Nouveau) — Abstraction Locator avec auto-waiting (visible, stable, enabled), filtrage sémantique et IA (`@semantic:`).
- `api/types.ts` (Nouveau) — `AnyPage` : interface commune pour la manipulation de profils asynchrones.
- `cli/serve.ts` — `bunlight serve --cdp-port N --profile P` (684 lignes)
- `recorder/TraceRecorder.ts` (Nouveau) — Enregistrement Zstd-compressé des actions, du réseau (HAR) et du DOM.
- `python-bridge/semantic_resolver.py` (Nouveau) — Résolution des sélecteurs flous par IA pour la phase 4.
...
### Trinity Architecture (High-Perf Hybrid)
- **Bun** (Orchestration): Main event loop, CDP transport, `bun:ffi`.
- **Rust** (Heavy Lifting): Native FFI for DOM/Markdown parsing (via `rust-bridge`).
- **Python** (AI/ML/RAG): Native FFI via `bun_python` + `uv`.
- **Concurrency**: Python tasks MUST use `runPythonWorker` to offload to a Bun Worker and bypass the GIL.

### Page Automation (2026 API)
- `click(selector)`: Native CDP mouse dispatch to element coordinates.
- `type(selector, text)`: Native CDP keyboard dispatch.
- `waitForSelector(selector, timeout)`: Polling-based readiness.
- `evaluate(fn)`: Executes JS in page context.
- `addCookies(cookies)`: Injects pre-authenticated session jars.
...
- [x] Stagehand-style AI scraper (déduit selectors avec LLM via python-bridge)
- [x] TraceViewer & Observabilité Zstd-native
