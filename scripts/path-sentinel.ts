...
async function scanAndFix() {
  ...
  const files = await $`find src test scripts examples -type f -name "*.ts" -o -name "*.js"`.text();
  ...
  for (const file of fileList) {
    ...
    if (homePattern.test(content)) {
      ...
      content = content.replace(homePattern, "${process.env.HOME || '${process.env.HOME || '/home/ubuntu'}'}");
    }

    // 2. Fix specific absolute paths to project root
    if (content.includes(PROJECT_ROOT)) {
      console.log(`  [root] Converting absolute project path to relative in ${file}`);
 ...
 */\n\n`;
  ...
  console.log(`✅ PATH SENTINEL: Task complete. Applied ${fixCount} corrections.`);
}
