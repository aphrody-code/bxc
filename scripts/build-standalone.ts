...
function getBinaryPathsForTarget(suffix: string): {
	...
	lightpanda: string | null;
	wappalyzergo: string | null;
} {
	...
	try {
		if (lightpanda && Bun.file(lightpanda).size <= 0) lightpanda = null;
	} catch {
		lightpanda = null;
	}

	const wappalyzergo = findWappalyzergo(suffix);

	return { rustBridge, curlImpersonate, lightpanda, wappalyzergo };
}

function findWappalyzergo(suffix: string): string | null {
	const root = join(import.meta.dir, "..");
	const vendor = join(root, "vendor", "wappalyzergo");
...
async function generateEmbeddedAssetsForTarget(t: BuildTarget): Promise<void> {
	...
	if (!rustBridge && !curlImpersonate && !lightpanda && !wappalyzergo) {
		console.log(
...
`;
	...
	if (lightpanda) console.log(`  - Lightpanda browser: ${lightpanda}`);
	if (wappalyzergo) console.log(`  - Wappalyzergo CLI: ${wappalyzergo}`);
...
`;
	...
	if (lightpanda) {
		content += `// @ts-ignore
import lightpandaAsset from ${JSON.stringify(lightpanda)} with { type: "file" };
export { lightpandaAsset };
`;
	} else {
		content += `export const lightpandaAsset = "";\n`;
	}

	if (wappalyzergo) {
		content += `// @ts-ignore
import wappalyzergoAsset from ${JSON.stringify(wappalyzergo)} with { type: "file" };
export { wappalyzergoAsset };
`;
	} else {
		content += `export const wappalyzergoAsset = "";\n`;
	}
