...
// Windows x64 follows the same baseline distinction (cf. https://bun.com/docs/project/building-windows).
const BASELINE = process.env.BUNLIGHT_BASELINE === "1";
const ALL_TARGETS: readonly BuildTarget[] = [
	{ target: BASELINE ? "bun-linux-x64-baseline" : "bun-linux-x64", suffix: "linux-x64" },
	{ target: "bun-linux-arm64", suffix: "linux-arm64" },
	{ target: BASELINE ? "bun-darwin-x64-baseline" : "bun-darwin-x64", suffix: "darwin-x64" },
	{ target: "bun-darwin-arm64", suffix: "darwin-arm64" },
	{ target: BASELINE ? "bun-windows-x64-baseline" : "bun-windows-x64", suffix: "windows-x64" },
];
...
const EXTERNALS: readonly string[] = ["electron", "playwright-core/lib/zipBundle"];
...
function pickTargets(): readonly BuildTarget[] {
	...
	if (env && env.length > 0) {
		...
		for (const w of wanted) {
			if (!known.has(w)) {
				throw new Error(`Unknown target "${w}" — valid : ${[...known].join(", ")}`);
			}
		}
...
async function buildOne(
	...
	const out = `${outDir}/bunlight-${t.suffix}`;
	...
	if (exitCode !== 0) {
		...
		return { target: t.target, out, ok: false, err: tail || `exit ${exitCode}` };
	}
	const exists = await Bun.file(out).exists();
	if (!exists) {
		return { target: t.target, out, ok: false, err: "outfile missing after build" };
	}
...
async function main(): Promise<void> {
	...
	if (targets.length === 0) {
		console.error("[build-standalone] No targets selected (empty BUNLIGHT_TARGETS ?)");
		process.exit(1);
	}
	console.log(
		`[build-standalone] version=${version} time=${buildTime} bytecode=${enableBytecode} baseline=${BASELINE}`,
	);
	console.log(`[build-standalone] Targets : ${targets.map((t) => t.suffix).join(", ")}`);
